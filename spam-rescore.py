...
SCORE_PAT = re.compile(r'.*score=([+-]?\d+\.?\d+).*')
REQUIRED_PAT = re.compile(r'.*required=(\d+\.?\d+).*')
...
def dateFromReceivedHeader(header):
  if header:
    vals = header.split(';')
    if len(vals) == 2:
      return datetime.fromtimestamp(mktime_tz(parsedate_tz(vals[1])))
      return vals[1]
  return None

TIME_PAT = re.compile(r'([-]?\d+)([dwmDWM])')
def parseSince(s):
  m = TIME_PAT.match(s)
  if m:
  ...
  def __init__(self, id, spamHeader, dateHeader, headers):
    ...
    if dateHeader:
      ...
      if isinstance(dateHeader, datetime):
        ...
        self.date = datetime.fromtimestamp(mktime_tz(parsedate_tz(dateHeader)))
      else:
        info(f'Msg CTOR unexpected date header type: {type(dateHeader)} value {dateHeader}')
...
def hydrateMessages(imap, messageIds, additionalFields=[]):
  ...
  parser = EmailParser()
  msgs = []
  for msgId, data in iter(messages.items()):
    #verbose(f'message data: {msgId} | {data}')
    rawHeaders = data.get(b'BODY[HEADER]', None)
    if rawHeaders is None:
      warn('no HEADER field on msg %s, skipping it' % str(msgId))
      continue
    rawHeaders = rawHeaders.decode('utf-8')
    headers = parser.parsestr(rawHeaders, True)
    verbose('parsed headers', type(headers))
    ...
    verbose('spam header', spamHeader, 'received header', dateHeader)
    for h in iter(headers.items()):
      verbose('header item', type(h), h)
    if not dateHeader:
      dateHeader = headers.get(b'Date', None)
    if not dateHeader:
      ...
      warn('message %s/"%s" has no discernible date, ignoring' % (msgId, headers.get('Subject', None)))
      continue
...
HOST_IP_PAT = re.compile(r'^from (\S+) \(\S+ \[(\d+\.\d+\.\d+\.\d+)\].*')
def hostAndIp(rcvd):
